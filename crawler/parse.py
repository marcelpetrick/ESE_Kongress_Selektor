...
def relative(path: Path) -> str:
    """Repo-relative when possible, absolute otherwise -- only for log output."""
    try:
...
# assembly
# --------------------------------------------------------------------------
def build(raw_dir: Path = RAW_DIR) -> dict:
    timetables = [parse_timetable(p) for p in sorted(raw_dir.glob("timetable_day_*.html"))]
    if not timetables:
        raise SystemExit(f"no timetable_day_*.html found in {raw_dir} -- run the crawler first")
...
def write(data: dict, json_path: Path = JSON_PATH, web_data_path: Path = WEB_DATA_PATH) -> None:
...
def main() -> int:
    ...
    parser.add_argument("--raw-dir", type=Path, default=RAW_DIR,
                        help="directory holding the crawled HTML (default: %(default)s)")
    parser.add_argument("--json", type=Path, default=JSON_PATH, dest="json_path",
                        help="where to write congress.json (default: %(default)s)")
    parser.add_argument("--web-data", type=Path, default=WEB_DATA_PATH, dest="web_data_path",
                        help="where to write the viewer payload (default: %(default)s)")
    args = parser.parse_args()

    data = classify.annotate(build(args.raw_dir))
    write(data, args.json_path, args.web_data_path)
    ...
    print(f"written   : {relative(args.json_path)}, {relative(args.web_data_path)}")
    return 0
